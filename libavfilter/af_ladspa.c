...
static av_cold int init(AVFilterContext *ctx, const char *args, void *opaque)
{
    ...
    if (soname[0] == '/' || soname[0] == '.') {
        ...
        char *ptr = paths;
        while (paths && !so) so = try_load(strsep(&paths, ":"), soname);
    ...
    if (!ladspa->nb_ins) {
        if (args && !strncmp(args, "rate", 4)) {
            arg = strsep(&args, ":");
            arg1 = strsep(&arg, "=");
            ladspa->sample_rate = ff_parse_sample_rate(arg1, ctx);
            if (ladspa->sample_rate == -1) return AVERROR(EINVAL);
        }
        if (!ladspa->sample_rate)
    ...
    while (arg = strsep(&args, ":")) {
        arg1 = strsep(&arg, "=");
        i = strtol(arg1+1, &tail, 10);
        if (!arg || arg1[0] != 'c' || *tail ||
            i < 0 || i >= ladspa->nb_ctls) {
            av_log(ctx, AV_LOG_ERROR,
                   "Bad control '%s'. Use "
                   "-af ladspa=%s:%s:help for a list of controls\n",
                   arg1, soname, ladspa->desc->Label);
            return AVERROR(EINVAL);
        }
        //FIXME check against hints or allow out of bounds values??
        ladspa->ctl_values[i] = strtod(arg, &tail);
...
static int config_output(AVFilterLink *outlink)
{
    ...
    if (ladspa->nb_ins)
        ...
        ladspa->outsamplesref = avfilter_get_audio_buffer(outlink,
...
static int request_frame(AVFilterLink *outlink)
{
    ...
    if (ladspa->nb_ins) {
        return avfilter_request_frame(outlink->src->inputs[0]);
    } else {
