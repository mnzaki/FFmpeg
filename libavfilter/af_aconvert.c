...
static void init_buffers(AVFilterLink *inlink, int nb_samples)
{
    ...
    int i, in_channels, out_channels;
    ...
    if (inlink->format != outlink->format ||
        ...
        for (i = 0; i < out_channels; i++) {
            aconvert->packed_data[i] = (uint8_t*)aconvert->packed_data+i;
    ...
    if (aconvert->audioconvert_ctx) {
        if (inlink->planar != outlink->planar) {
            if (!aconvert->mix_samplesref) {
                if (outlink->planar)
                    while (nb_channels--)
                        aconvert->packed_data[nb_channels] =
                                    curbuf->data[0] + nb_channels;
                else
        ...
        av_audio_convert(aconvert->audioconvert_ctx,
                         (void * const *) aconvert->in_data,
                         aconvert->out_strides,
                         (const void * const *) aconvert->out_data,
                         aconvert->in_strides,
