...
typedef struct {
    int nb_samples;                         ///< number of buffered samples
...
static av_cold int init(AVFilterContext *ctx, const char *args, void *opaque)
{
    ...
    if (args)
        sscanf(args, "%7[^:]:%31s", sample_fmt_str, chlayout_str);
    ...
    if (*sample_fmt_str && aconvert->out_sample_fmt == AV_SAMPLE_FMT_NONE) {
        /* AV_SAMPLE_FMT_NONE is a valid value for out_sample_fmt and indicates
         * no change in sample format. */
        char *tail;
...
static av_cold void uninit(AVFilterContext *ctx)
{
    ...
    aconvert->mix_samplesref = NULL;
    aconvert->out_samplesref = NULL;
