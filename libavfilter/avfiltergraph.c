                              ...
                              const char *filt_name, const char *filt_args)
{
    ...
    if (link->type == AVMEDIA_TYPE_AUDIO &&
        ...
        av_log(NULL, AV_LOG_ERROR,
               "Impossible to convert between the channel layouts supported by the filter "
               "'%s' and the filter '%s'\n", link->src->name, link->dst->name);
        return AVERROR(EINVAL);
    }
    ...
    for (i = 0; i < graph->filter_count; i++) {
        ...
        for (j = 0; j < filter->input_count; j++) {
            ...
            if (link->type == AVMEDIA_TYPE_VIDEO &&
                ...
                /* Allow filters to get away with not setting any chlayouts */
                if (!link->in_chlayouts)
                ...
                if ((link->in_packing & link->out_packing)        ||
