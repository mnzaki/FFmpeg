...
static av_cold int init(AVFilterContext *ctx, const char *args, void *opaque)
{
    ...
    abuffer->channel_layout = av_get_channel_layout(ch_layout_str);
    if (abuffer->channel_layout < AV_CH_LAYOUT_STEREO) {
        char *tail;
        abuffer->channel_layout = strtol(ch_layout_str, &tail, 0);
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    int64_t channel_layouts[] = { abuffer->channel_layout, -1 };
