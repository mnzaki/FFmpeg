...
static int configure_audio_filters(VideoState *is, const char *afilters)
{
    ...
    if ((ret = avfilter_graph_create_filter(&asrc_ctx, &input_audio_filter, "asrc",
                                            NULL, is, is->agraph)) < 0)
        goto fail;
    if ((ret = avfilter_graph_create_filter(&aout_ctx, &ffasink, "aout",
                                            NULL, &ffasink_ctx, is->agraph)) < 0)
        goto fail;
...
#if CONFIG_AVFILTER
            get_filtered_audio_buffer(is->out_audio_filter,
